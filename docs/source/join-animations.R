...
lj <- animate(lj)
anim_save(here::here("images/animation-left-join.gif"), lj)

# Left Join 2 ----
y2 <- bind_rows(y, data_frame(id = 2, y = "y5"))
initial_dfs2 <- x %>%
  proc_data("x") %>%
  {
    extra <- filter(., value == "x2")
    bind_rows(., extra)
  } %>%
  bind_rows(mutate(proc_data(y2, "y"), .x = .x + 3)) %>%
  mutate(frame = 1)

joined_dfs <- left_join(x, y2, "id") %>%
  proc_data("x") %>%
  mutate(frame = 2, .x = .x + 1)

extra_blocks <- inner_join(x, y2, "id") %>%
  select(id) %>%
  proc_data("y") %>%
  mutate(frame = 2, .x = .x + 1)

# Just gonna hack it right now
...
lj2 <- animate(lj2)
anim_save(here::here("images/animation-left-join2.gif"), lj2)
...
rj <- animate(rj)
anim_save(here::here("images/animation-right-join.gif"), rj)
...
ij <- animate(ij)
anim_save(here::here("images/animation-inner-join.gif"), ij)
...
fj <- animate(fj)
anim_save(here::here("images/animation-full-join.gif"), fj)
...
sj <- animate(sj)
anim_save(here::here("images/animation-semi-join.gif"), sj)
...
aj <- animate(aj)
anim_save(here::here("images/animation-anti-join.gif"), aj)
